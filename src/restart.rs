...
fn decode_descriptor(frame: &[u8; FRAME_LEN]) -> io::Result<DescriptorRole> {
    ...
    Ok(match frame[8] {
        ...
        PUBLIC_HTTP_FD => DescriptorRole::PublicHttp(address),
...
fn receive_frame(socket: BorrowedFd<'_>) -> io::Result<ProtocolMessage> {
    ...
    match frame[8] {
        PUBLIC_FD | ADMIN_FD | TCP_FD | ACME_HTTP_FD | WORKLOAD_HTTP_FD | PUBLIC_HTTP_FD => {
