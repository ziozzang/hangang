#!/usr/bin/env python3
"""Stage reproducible Linux amd64 release assets without publishing them."""
...
def prepare(seed: Path, output: Path, target: str) -> None:
    if target != TARGET:
        raise ValueError(f"unsupported target: {target}; only {TARGET} is supported")
    if not seed.is_file() or seed.is_symlink():
        raise ValueError("signing seed must be an existing regular file")
    seed = seed.resolve()
    output = output.resolve()
    if output == seed or output in seed.parents or seed in output.parents:
...
def main() -> int:
    ...
    try:
        prepare(args.signing_seed_file, args.output, args.target)
