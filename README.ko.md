...
macOS Intel이나 Apple Silicon에서 개발·시연하고, 동일한 설정 모델을 Linux x86-64 또는 ARM64에 배포하세요. 네이티브 빌드 매트릭스는 네 대상을 포함합니다. **감독 프로세스 기반 업그레이드, syscall로 격리된 Lua, IPVS DSR의 기준 환경은 Linux입니다.** macOS는 제한을 명시한 일반 게이트웨이 실행을 제공하며, 신뢰하는 개발용 Lua에는 `--allow-unsandboxed-lua`가 필요합니다. 네이티브 Windows 바이너리는 제공하지 않으며 WSL2 같은 Linux 환경을 사용해야 합니다. 검증 조건과 제한은 [플랫폼 지원과 릴리스 대상](docs/ko/PLATFORMS.md)을 참고하세요.
