...
[English](README.md) · [문서 목차](docs/README.ko.md) · [빠른 시작](#빠른-시작)

**동적 설정, 격리된 Lua 정책, 웹 관리 화면을 하나로 제공하는 HTTP·TCP 게이트웨이입니다.**
...
공식 문서 확인 기준은 **2026-09-22**입니다. 에디션·모듈·설정에 따라 기능 범위가 달라지며, 이 표는 처리량 순위가 아니라 운영 방식의 비교입니다. Hangang은 UDP/QUIC 라우팅을 구현하지 않았고, 관리자 계정은 인스턴스별로 보관합니다. 이전 전 [아키텍처](docs/ARCHITECTURE.md)와 [다중 인스턴스 운영](docs/ko/SCALE_OUT.md)을 확인하세요.
